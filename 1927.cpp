...
private:
    ...
    void siftdown(int target_idx) {
        ...
        if (right_child != -1) {
            int MIN = min(container[left_child], container[right_child]);
            if (container[target_idx] > MIN && MIN == container[left_child]) {
                int tmp = container[target_idx];
                container[target_idx] = container[left_child];
                container[left_child] = tmp;

                siftdown(left_child);
            } else if (container[target_idx] > MIN && MIN == container[right_child]) {
                int tmp = container[target_idx];
